...
/// <summary>
/// JSON Schema Draft 05 implementation for OpenAPI 3.0
/// </summary>
public sealed class JsonSchemaDraft05 : IJsonSchema
{
    ...
    /// <summary>
    /// Instantiate from a JSON Schema draft 05 (OpenAPI 3.0)
    /// </summary>
    /// <param name="schema">JSON Schema</param>
    public JsonSchemaDraft05(JsonNode? schema)
    {
    ...
    {
        _jsonValueType ??= new Lazy<InstanceType?>(() =>
        {
            ...
            if (type != null && schemaObject?["nullable"]?.GetValue<bool>() == true)
            {
    ...
    public IJsonSchema? GetItems()
    {
        _items ??= new Lazy<IJsonSchema?>(() =>
        {
            ...
            return items == null ? null : new JsonSchemaDraft05(items);
        });
        return _items.Value;
    }
    ...
    public IReadOnlyDictionary<string, IJsonSchema>? GetProperties()
    {
        _properties ??= new Lazy<IReadOnlyDictionary<string, IJsonSchema>?>(() =>
        {
            ...
            return properties.ToImmutableDictionary<KeyValuePair<string, JsonNode?>, string, IJsonSchema>(
                pair => pair.Key,
                pair => new JsonSchemaDraft05(pair.Value));
        });
